/* Harnais de validation — Flyers de Philadelphie (jsdom) */
const fs = require('fs');
...
const ATTENDU = {
  equipe: 'PHILLY',
  taille: 24,
  echantillon: {nom: 'Auston Matthews', salaire: 12200000, ct: 1, ov: 84},
  masseSousContrat: 53325000,
  nbSousContrat: 9
};
...
(async () => {
  ...
  const vAho = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Sebastian Aho'), 'UFA', 1);
  egal(vAho.code, 'surpaye', 'Sebastian Aho (OV 84, 29 ans, 10,625 M, UFA) → Surpayé');
  egal(vAho.minEff, 10250000, 'Charte Aho : 10 250 000 $');
  egal(vAho.ecart, 375000, 'Écart Aho : +375 000 $');
  const vKul = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Jiri Kulich'), 'RFA', 1);
  egal(vKul.code, 'aubaine', 'Jiri Kulich (OV 79, 22 ans, 900 k, RFA) → Aubaine salariale');
  egal(vKul.ecart, 2850000, 'Écart Kulich : −2 850 000 $ sous la charte (3 750 k)');
  ...
  const vLev = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Devon Levi'), 'RFA', 1);
  egal(vLev.minEff, 3750000, 'Devon Levi (G, OV 80, 25 ans, RFA) évalué à l\'échelon 79 : 3 750 000 $');
  egal(vLev.code, 'aubaine', 'Devon Levi à 3,125 M → Aubaine salariale');
  ...
  selJoueur.value = 'Sebastian Aho';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Aho sélectionné : badge Surpayé');
  ...
  egal(celluleActive.textContent.replace(/[\s\u00a0\u202f]/g,''), '10250$',
    'Cellule active : 10 250 $ (UFA 34-, OV 84)');
  ...
  selJoueur.value = 'Devon Levi';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('aubaine'), 'Devon Levi : badge Aubaine salariale');
