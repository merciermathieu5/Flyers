...
(async () => {
  ...
  egal(S.SECOURS_ROSTER.length, 24, '24 joueurs dans la formation de secours (21 patineurs + 3 gardiens, page du 21 avril)');
  const aho = S.SECOURS_ROSTER.find(j => j.nom === 'Sebastian Aho');
  egal(aho.salaire, 8500000, 'Salaire Aho');
  egal(aho.ov, 83, 'Aho OV 83');
  const tka = S.SECOURS_ROSTER.find(j => j.nom === 'Brady Tkachuk');
  egal(tka.salaire, 9750000, 'Salaire Tkachuk');
  egal(tka.ov, 85, 'Tkachuk OV 85');
  egal(S.SECOURS_Y21.length, 20, '20 patineurs dans la référence Y21 (XtraStats, validés par ratios)');
  const ahoY21 = S.SECOURS_Y21.find(x => x.nom === 'Sebastian Aho');
  egal(ahoY21.pts, 100, 'Aho 100 points en Y21');
  const tkaY21 = S.SECOURS_Y21.find(x => x.nom === 'Brady Tkachuk');
  egal(tkaY21.goals, 50, 'Tkachuk 50 buts en Y21');
  egal(tkaY21.hits, 229, 'Tkachuk 229 MEÉ en Y21');
  ok(!S.SECOURS_Y21.find(x => x.nom === 'Matt Grzelcyk'),
     'Grzelcyk absent de la référence Y21 (aucune ligne PHILLY dans XtraStats — rien d\'inventé)');
  ...
  egal(profilDe('Sebastian Aho').profil, 'Elite', 'Aho → Elite');
  tableauEgal(profilDe('Sebastian Aho').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Auston Matthews').profil, 'Elite', 'Matthews → Elite');
  egal(profilDe('Brady Tkachuk').profil, 'Power Forward', 'Tkachuk (OV 85) → Power Forward');
  egal(profilDe('Sean Couturier').profil, 'Playmaker', 'Couturier → Playmaker');
  tableauEgal(profilDe('Sean Couturier').stats, ['assists','pts'], 'Stats Playmaker = A, P');
  egal(profilDe('Jon A.Marchessault').profil, 'Two-Way Forward', 'Marchessault → Two-Way Forward');
  egal(profilDe('Jake Guentzel').profil, 'Sniper', 'Guentzel → Sniper');
  egal(profilDe('Nino Niederreiter').profil, 'Sniper', 'Niederreiter → Sniper');
  egal(profilDe('Pius Suter').profil, 'Sniper', 'Suter → Sniper');
  egal(profilDe('Nic Dowd').profil, 'Grinder', 'Dowd → Grinder');
  tableauEgal(profilDe('Nic Dowd').stats, ['hits20'], 'Stats Grinder = MEÉ/20');
  egal(profilDe('Lars Eller').profil, 'Defensive', 'Eller → attaquant défensif');
  egal(profilDe('J-J Peterka').profil, 'Prospect Elite', 'Peterka (23 ans) → Prospect Elite');
  egal(profilDe('J-J Peterka').mat, 'ELITE', 'Prospect Elite → même matrice ELITE');
  egal(profilDe('Jiri Kulich').profil, 'Prospect Two-Way Forward', 'Kulich (21 ans) → Prospect Two-Way');
  egal(profilDe('Adam Boqvist').profil, 'DOffensive', 'Boqvist → DOffensive');
  egal(profilDe('Matt Grzelcyk').profil, 'DOffensive', 'Grzelcyk → DOffensive');
  egal(profilDe('Brady Skjei').profil, 'DDefensive', 'Skjei → DDefensive');
  egal(profilDe('Ben Chiarot').profil, 'DDefensive', 'Chiarot → DDefensive (99 d\'expérience!)');
  egal(profilDe('Domenick Fensore').profil, 'Prospect DEliteQB', 'Fensore (24 ans) → Prospect DEliteQB');
  egal(profilDe('Devon Levi').profil, 'Starter Goalie', 'Levi (OV 80, 24 ans) → Starter Goalie (règle 6.2.5, peu importe l\'âge)');
  tableauEgal(profilDe('Devon Levi').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Joseph Woll').profil, 'Starter Goalie', 'Woll (OV 81) → Starter Goalie');
  egal(profilDe('Drew Commesso').profil, 'Backup Goalie', 'Commesso (OV 78) → Backup Goalie');
  tableauEgal(profilDe('Drew Commesso').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const jk = joueurDe('Sebastian Aho');
  ...
  const fixtureXtra = [
    ...
    '    [Sebastian Aho](https://x/#Sebastian Aho) PHILLY          C   82  30  45  75 200  30 1700 100 2.44',
    '    [Pierre-Luc Dubois](https://x/#Pierre-Luc Dubois) ANAHEIM         C   76  30  38  68 199  23 1713 137 2.87',
    '    [* Backup_RW](https://x/#Backup_RW) PHILLY          LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'PHILLY');
  egal(x.length, 2, 'Deux patineurs PHILLY extraits (Anaheim exclu)');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('PHILLY'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 82860000, 'Masse salariale des 24 pros = 82 860 000 $ (vérifiée contre la moyenne de la page : 3 452 500 $ × 24)');
  ...
  console.log('— Divers');
  egal(S.matchsEquipe(), 82, 'Fiche PHILLY 49-30-3 → 82 matchs (saison Y21 complète, page du 21 avril)');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Sebastian Aho', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jAho = S.SECOURS_ROSTER.find(x2=>x2.nom==='Sebastian Aho');
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Sebastian Aho'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 100, 'Points Y21 d\'Aho');
  const carteAho = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Sebastian Aho');
  ok(!!carteAho, 'Carte d\'Aho rendue en mode Y21');
  ok(carteAho.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jAh = S.ETAT.roster.find(x2=>x2.nom==='Sebastian Aho');
  const evPtsA = S.evaluerStat('pts', S.valeurStat(jAh, prodY21, 'pts', {}), S.seuilsPour(jAh, 'pts', {}, {}).seuils);
  egal(evPtsA.statut, 'memorable', 'Aho MÉMORABLE en points (100 > 97, Elite OV 83) — saison de 100 points');
  const jTk = S.ETAT.roster.find(x2=>x2.nom==='Brady Tkachuk');
  const evButsT = S.evaluerStat('goals', S.valeurStat(jTk, S.productionDe(jTk), 'goals', {}), S.seuilsPour(jTk, 'goals', {}, {}).seuils);
  egal(evButsT.statut, 'memorable', 'Tkachuk MÉMORABLE en buts (50 > 37, Power Forward OV 85)');
  const jMt = S.ETAT.roster.find(x2=>x2.nom==='Auston Matthews');
  const evPtsMt = S.evaluerStat('pts', S.valeurStat(jMt, S.productionDe(jMt), 'pts', {}), S.seuilsPour(jMt, 'pts', {}, {}).seuils);
  egal(evPtsMt.statut, 'correcte', 'Matthews Correcte en points (73 < 84 ≤ 85, Elite OV 84) — à un point de Satisfaisante');
  const prodGrz = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Matt Grzelcyk'));
  ok(!prodGrz, 'Grzelcyk sans repère Y21 : aucune production servie en mode vérification (rien d\'inventé)');
  ok(!carteAho.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
